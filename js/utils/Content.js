if(window.Selector == null) {
	console.log("Content.js requires Selector.js");
	if(window.Require)
        Require.script("Selector.js");
}
if(window.EventDispatcher == null) {
	console.log("Content.js requires EventDispatcher.js");
    if(window.Require)
	   Require.script("EventDispatcher.js");
}
/** Class: Content
...
function Content(builderID, database, dynamicLoad) {
	...
	this.getContent = function(filter, key, tempDB) {
		...
		for(var i = 0; i < dbLength; ++i){
			...
			if(isFound) {
				...
				var item = Selector.create(this.contentTag, id + "-" + (list.length - 1));
				Selector.addClass(item, "content-item");
				Selector.addClass(item, id);
				item.innerHTML = this.buildContent(tempDB[i], this.getDesign());
				
				var temp = Selector.create(Selector.type.DIV);
	...
	*/
	this.deSelect = function(dataID) {
		var target = Selector.el(id + "-" + dataID);
		if(target)
			Selector.removeClass(target, "selected");
	}
	/** Function: selectByID
        select item by id
	...
	*/
	this.selectByID = function(dataID) {
		var target = Selector.el(id + "-" + dataID);
		if(target)
			Selector.addClass(target, "selected");
	}
	/** Function: onItemOut
	...
	*/
	this.deActivate = function(dataID) {
		var target = Selector.el(id + "-" + dataID);
		if(target)
			Selector.removeClass(target,"active");
	}
	/** Function: activeByID
	...
	*/
	this.activeByID = function(dataID) {
		var target = Selector.el(id + "-" + dataID);
		if(target)
			Selector.addClass(target, "active");
	}
	...
	*/
	this.showContent = function() {
		this.addLiveListener(id, function(){
				Selector.el(id).style.display = "block";
	...
	*/
	this.hideContent = function() {
		this.addLiveListener(id, function(){
				Selector.el(id).style.display = "none";
